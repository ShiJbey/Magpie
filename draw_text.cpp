...
Load< MeshBuffer > text_meshes(LoadTagInit, [](){
	return new MeshBuffer(data_path("menu.pnc"));
});
...
GLint icon_program_mvp_mat4 = -1;
Load< GLuint > icon_color_program(LoadTagInit, [](){
    ...
    icon_program_mvp_mat4 = glGetUniformLocation(*ret, "mvp");
    return ret;
});
...
Load< GLuint > text_meshes_for_icon_program(LoadTagDefault, [](){
    return new GLuint(text_meshes->make_vao_for_program(*icon_color_program));
});
...
void draw_object(std::string const &text, glm::vec2 const &anchor, float height) {
	...
	for (uint32_t i = 0; i < text.size(); ++i) {
		...
		if (text[i] != ' ') {
			...
			glUniformMatrix4fv(icon_program_mvp_mat4, 1, GL_FALSE, glm::value_ptr(mvp));
