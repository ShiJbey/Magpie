...
#blender --background --python export-anims.py -- <infile.blend> <layer> <outfile.kani>
...
def write_xfh(obj):
	...
	if anim:
		if anim.action:
			print(obj.name,'uses',anim.action.name)

			# Break out the fcurves for each transform component
			fcurves = anim.action.fcurves
			w_quat_rot = fcurves[0]
			x_quat_rot = fcurves[1]
			y_quat_rot = fcurves[2]
			z_quat_rot = fcurves[3]
			x_location = fcurves[4]
			y_location = fcurves[5]
			z_location = fcurves[6]
			x_scale = fcurves[7]
			y_scale = fcurves[8]
			z_scale = fcurves[9]

			# DANGEROUS:: Assume that all fcurves have the same number of key points
			num_frames = len(w_quat_rot.keyframe_points)

			# Loop through each of the keyframes
			for frame in w_quat_rot.keyframe_points:
				# Advance the scene
				bpy.context.scene.frame_set(round(frame.co[0]))
				transform = (world_to_parent * obj.matrix_world).decompose()
				# Write out the current transform
				xfh_data += parent_ref
				xfh_data += write_string(obj.name)
				xfh_data += struct.pack('I', round(frame.co[0]))
			...
			#for fc in anim.action.fcurves:
				#print ("fcurve ({0}) has ({1}) keys".format(fc.array_index ,len(fc.keyframe_points)))
				#for key in fc.keyframe_points:
					#print('frame:',round(key.co[0]))
					#bpy.context.scene.frame_set(round(key.co[0]))
